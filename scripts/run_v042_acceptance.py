...
def known_findings(result: Any) -> list[str]:
    ...
    lines.append(
        "5. **A hard break inside a projected paragraph is not rebuilt as a "
        "break.** The projection emits it as a `<br>` in the Canonical Author "
        "document, and `br` is not part of the declared inline-element surface, "
        "so the New Deck path rebuilds the two sides as one paragraph with no "
        "character lost. Recorded by the pytest module's "
        "`test_the_probe_b_hard_break_is_emitted_and_the_rebuild_merges_it`."
    )
    lines.append(
        "6. **A source list's native marker is not reconstructed.** OfficeCLI "
        "reads the synthetic probe's list block back with `list=bullet` and a "
        "native `a:buChar` marker; the rebuilt deck's object is plain paragraphs, "
        "because the Canonical Author surface has no list element. The item text "
        "round-trips; the marker does not. Recorded by the pytest module's "
    ...
    lines.append(
        "8. **Proxy target survival is asserted from the rebuilt deck, not "
        "measured from the proxy's pixels.** The isolation proof establishes "
        "that a picture object with the proxy's own name exists in the rebuilt "
        "deck and that its raster has the right density, bounds, guard band and "
        "contamination; it does not establish that the locked object's own paint "
        "is in that raster. "
        ...
        "This is the same limitation the independent verification report records "
        "for the gate."
    )
    lines.append(
