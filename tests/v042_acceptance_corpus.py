...
#: The eight real source pages, in selection order.  Frozen: no substitution.
REAL_PAGES: tuple[int, ...] = (2, 5, 7, 12, 14, 19, 25, 30)
...
SELECTION_ORDER: tuple[str, ...] = tuple(
    [f"real:{page}" for page in REAL_PAGES] + [PROBE_A_KEY, PROBE_B_KEY]
)
...
#: Composition and coverage of the eight frozen real pages, as measured on the
#: source deck when the selection was frozen.  These are literals a reviewer can
#: check against the source deck; nothing here is read out of a projection.
_REAL_COVERAGE: Mapping[int, tuple[str, str, str]] = {
    2: (
        "25 textbox, 6 shape (3 ellipse, 3 rightArrow), 2 group, 4 connector, "
        "4 picture; 1 source overflow",
        "groups + bound connectors, native ellipse/rightArrow, pictures, text "
        "density, repeated cards, source-inherent overflow",
        ...
        "are never emitted again as siblings. The one text overflow is the "
        "source's own condition and stays a retained finding.",
    ),
    5: (
        "2 picture, 8 shape, 1 table",
        "native table, roundRect, transparent/cropped pictures",
        ...
        "evidence, never slide-owned projection loss.",
    ),
    7: (
        "1 shape, 1 table, 1 textbox",
        "large native table (11x6) with whitespace-placement cell findings",
        "Two table cells differ from the source in whitespace placement only; "
        "the condition is retained and reported, never repaired and never "
        "counted as a material delta. One shape's text colour is a theme "
        "expression, so it is base-only-semantic.",
    ),
    12: (
        "44 shape, 1 table; 13 source issues",
        "high object density, gradient fills, many source-inherent overflows",
        "This page carries the corpus's highest source-issue count and its "
        "gradient fills. Every issue is the source's own; none is introduced by "
        "the projection. Gradient-filled objects are not canonical-editable and "
        "keep their locked or base-only disposition.",
    ),
    14: (
        "5 shape, 3 table",
        "compound/multi-table page",
        "Three native tables on one page exercise per-table identity and "
        "independent row/column and cell-text readback.",
    ),
    19: (
        "7 shape, 1 table, 6 picture, 1 textbox",
        "the most picture-heavy page; source-inherent overflow",
        "Six picture objects exercise the picture surface and the one table "
        "exercises native table readback alongside them. The overflow is the "
        "source's own condition.",
    ),
    25: (
        "7 shape, 19 textbox, 13 picture, 2 connector",
        "mixed density with a standalone connector",
        "A standalone connector is locked, not reconstructed natively: the "
        "current Contract has no editable connector object. It is one locked "
        "proxy or one retained finding, never a silently dropped object.",
    ),
    30: (
        "30 shape, 16 picture, 7 connector, 4 textbox",
        "highest object count (57); connector-heavy",
        "The densest page in the corpus. Connector-heavy composition is what "
        "exercises the container/locked boundary at scale: every connector has "
        "exactly one disposition in the ledger.",
    ),
}

_SOURCE_KIND_REAL = "private business deck (local, not published)"
_SOURCE_KIND_SYNTHETIC = "committed synthetic probe built through OfficeCLI"


#: The source page each corpus page id comes from.
PAGE_OF_PAGE_ID: Mapping[str, int] = {
    **{f"real:{page}": page for page in REAL_PAGES},
    PROBE_A_KEY: PROBE_A_PAGE,
    PROBE_B_KEY: PROBE_B_PAGE,
}


def _pages() -> tuple[CorpusPage, ...]:
    pages: list[CorpusPage] = []
    for order, page in enumerate(REAL_PAGES, start=1):
        composition, purpose, note = _REAL_COVERAGE[page]
        pages.append(
            CorpusPage(
                order=order,
                page_id=f"real:{page}",
...
def corpus_manifest(
    *,
    ...
    ``source_hashes`` is keyed by page id for real pages and carries the sha256
    of the private source deck; it is written into the acceptance bundle and
    deliberately kept out of the repository's committed text.
    """
    ...
    for page in CORPUS_PAGES:
        ...
        if page.synthetic:
            ...
            record["source_sha256"] = source_hashes.get(page.page_id)
        pages.append(record)
    manifest: dict[str, Any] = {
        "schema_version": CORPUS_SCHEMA_VERSION,
        "selection_order": list(SELECTION_ORDER),
        "real_page_count": len(REAL_PAGES),
        "synthetic_probe_count": len(CORPUS_PAGES) - len(REAL_PAGES),
        "pages": pages,
    }
...
def seam_selection(
    baseline_deck: Path | None,
    probe_a: Path | None,
    probe_b: Path | None,
) -> list[tuple[str, int]]:
    """Return the frozen corpus's ``(deck, page)`` pairs in selection order.

    The full corpus is ten pages: the eight real ones, then both pages of probe
    A, then probe B's page.  A ``None`` deck drops its pages, which is how the
    always-run synthetic half of the acceptance test drives the same seam over
    the two probes alone.
    """
    selection: list[tuple[str, int]] = []
    if baseline_deck is not None:
        selection.extend((str(baseline_deck), page) for page in REAL_PAGES)
...
def expected_page_sequence(
    baseline_present: bool, probes_present: bool = True
) -> tuple[str, ...]:
    """The page ids the seam selection must produce, in order."""
    sequence = [f"real:{page}" for page in REAL_PAGES] if baseline_present else []
    if probes_present:
